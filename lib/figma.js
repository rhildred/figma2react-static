...
const getNodeIds = (child) =>{
  let sRc = `${child.id}`;
  for(let n = 0; child.children && n < child.children.length; n++){
    sRc += `,${getNodeIds(child.children[n])}`;
  }
  return(sRc);
};
...
`;
  ...
  const visitNode = (imgMap, node, parent, lastVertical, indent) => {
    ...
    if (node.id !== component.id && node.type === 'FRAME') {
      ...
      if (node.transitionNodeID && parent && parent.name != node.name) {
        let sTo = node.name.toLowerCase();
        if(node.name == componentMap.index){
          sTo = "/";
        }
        print(`    <Link to="${sTo}">`, indent);
        fs.writeFileSync(`${node.name}_${node.id}_link.json`, JSON.stringify(node));
      }else{
        print(`    <div>`, indent);
      }
...
module.exports = { createComponent, colorString, getNodeIds }
